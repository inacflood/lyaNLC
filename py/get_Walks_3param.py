...
if not testingBB:
    ...
    for q1,q2,av in samples[np.random.randint(len(samples), size=200)]:
        plt.plot(k, th.FluxP1D_hMpc(z, k*dkMz, q1=q1, q2=q2, kp=kp_f, kvav=kvav_f, av=av, bv=bv_f)*k_res/np.pi
                 , color="b", alpha=0.1)
...
if testingBB:
   ...
   cornerplt = corner.corner(samples, labels=[ "$q1$", "$q2$", "$av$"],
                truths=[q1_f,q2_f,av_f],quantiles=[0.16, 0.5, 0.84],show_titles=True)
   
   cornerplt.savefig("../output/"+headFile+"/triangle_err"+err_str+"posFSmtT.pdf")
   cornerplt.show()
   v1_mcmc, v2_mcmc, v3_mcmc = map(lambda v: 
               (v[1], v[2]-v[1], v[1]-v[0]),zip(*np.percentile(samples, [16, 50, 84], axis=0)))
    
   print("q1:", v1_mcmc, "q2:", v2_mcmc, "kp:", v3_mcmc) 
