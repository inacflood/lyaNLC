...
# -*- coding: utf-8 -*-
"""
Created on Wed May 29 14:57:39 2019

@author: iflood
"""
import numpy as np
import lya_kaiser_uv_zevol_mod as kailya
...
class TheoryLyaP3D(object):
    ...
    def FluxP3D_hMpc(self,z,k_hMpc,mu,linear=False,uv=True,zevol=True, 
        ...
        if (uv==True) and (zevol==True) :
            kaiser = kailya.Kaiser_LyA_hMpc(k, mu, z, beta_lya, b_lya, b_g = 0.13, b_sa = 1, b_a = -2./3, k0 = 300, a_lya=2.9)
        if (uv==False) and (zevol==True) :
            kaiser = kailya.Kaiser_LyA_hMpc(k, mu, z, beta_lya, b_lya, b_g = 0., b_sa = 1, b_a = -2./3, k0 = 300, a_lya=2.9)
        if (uv==True) and (zevol==False) :
            kaiser = kailya.Kaiser_LyA_hMpc(k, mu, z, beta_lya, b_lya, b_g = 0.13, b_sa = 1, b_a = -2./3, k0 = 300, a_lya=0)
        if (uv==False) and (zevol==False) :
            kaiser = kailya.Kaiser_LyA_hMpc(k, mu, z, beta_lya, b_lya, b_g = 0., b_sa = 1, b_a = -2./3, k0 = 300, a_lya=0.)
