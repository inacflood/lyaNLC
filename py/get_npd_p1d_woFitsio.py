...
class LyA_P1D():
    ...
    def __init__(self,z_bin,method=0):
        ...
        if method == 0 :
            path_to_fitsfile = "../NPD2013_data/table4a.dat"
            n_kbin=35

        if method == 1 :
            path_to_fitsfile = "../NPD2013_data/table5a.dat"
            n_kbin=32
            
        z,k,Pk,Pk_stat,Pk_noise,Pk_metal,Pk_syst = np.loadtxt(path_to_fitsfile, unpack=True, usecols=range(2,9))
...
#        Pk_syst=fitsio.read(path_to_fitsfile,columns="Pk_syst")
    ...
    def Pk_emp(self):
        """
            Returns the empirical function kPk_emp( k ; z_bin )/pi which fits each power spectrum distribution. 
