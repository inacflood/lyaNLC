...
def plot_3D(z,mu=1.0,uv=False,zevol=True):
    ...
    plt.plot(k, k * model_lin / np.pi, color='r', linewidth=2 ,label=r'linear')
    plt.plot(k, k * model_nonlin / np.pi, color='b', linewidth=2 ,label=r'nonlinear')

    plt.legend(loc='best')
    plt.savefig("../Figures/P3D_lin_and_nonlin.pdf")
