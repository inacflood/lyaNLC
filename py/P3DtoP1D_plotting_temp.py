...
p3t1_22 = th22.makeP1D_I(z22, linear=linear)#,linear=True) # 1D power spectrum from manual Riemannian sum
#p3t1_22_t = th22.makeP1D_T(z22)
p3t1_22_p = th22.makeP1D_P(kpa_list,z22, linear=linear)             # 1D power spectrum from np.trapz integration
...
p3t1_24 = th24.makeP1D_I(z24, linear=linear)#,linear=True)
#p3t1_24_t = th24.makeP1D_T(z24) 
p3t1_24_p = th24.makeP1D_P(kpa_list,z24, linear=linear) 
...
p3t1_26 = th26.makeP1D_I(z26, linear=linear)#,linear=True)
#p3t1_26_t = th26.makeP1D_T(z26) 
p3t1_26_p = th26.makeP1D_P(kpa_list,z26, linear=linear)  
...
p3t1_28 = th28.makeP1D_I(z28, linear=linear)#,linear=True)
#p3t1_28_t = th28.makeP1D_T(z28)
p3t1_28_p = th28.makeP1D_P(kpa_list,z28,linear= linear)   
...
p3t1_30 = th30.makeP1D_I(z30, linear=linear)#,linear=True)
#p3t1_30_t = th30.makeP1D_T(z30) 
p3t1_30_p = th30.makeP1D_P(kpa_list,z30, linear=linear)  
...
plt.plot(p3t1_24[0]/dkM24(z24) , p3t1_24[0] * p3t1_24_p / np.pi,'saddlebrown',alpha=0.7, linewidth=2 ,label=r'$z = 2.4 $, ptwise')
plt.plot( npd24.k , npd24.k/np.pi*(npd24.Pk) , 'r^', alpha=0.4, linewidth=2, label=r'$z=2.4$, npd')

plt.plot(p3t1_22[0]/dkM22(z22) , p3t1_22[0] * p3t1_22[1] / np.pi,'b',alpha=0.7, linewidth=2 ,label=r'$z = 2.2 $, th')
plt.plot(p3t1_22[0]/dkM22(z22) , p3t1_22[0] * p3t1_22_p / np.pi,'navy',alpha=0.7, linewidth=2 ,label=r'$z = 2.4 $, ptwise')
plt.plot( npd22.k , npd22.k/np.pi*(npd22.Pk) , 'b^', alpha=0.4, linewidth=2, label=r'$z=2.2$, npd')

plt.plot(p3t1_26[0]/dkM26(z24) , p3t1_26[0] * p3t1_26[1] / np.pi,'g',alpha=0.7, linewidth=2 ,label=r'$z = 2.6 $, th')
plt.plot(p3t1_26[0]/dkM26(z24) , p3t1_26[0] * p3t1_26_p / np.pi,'lawngreen',alpha=0.7, linewidth=2 ,label=r'$z = 2.6 $, ptwise')
plt.plot( npd26.k , npd26.k/np.pi*(npd26.Pk) , 'g^', alpha=0.4, linewidth=2, label=r'$z=2.6$, npd')

plt.plot(p3t1_28[0]/dkM28(z28) , p3t1_28[0] * p3t1_28[1] / np.pi,'k',alpha=0.7, linewidth=2 ,label=r'$z = 2.8 $, th')
plt.plot(p3t1_28[0]/dkM28(z28) , p3t1_28[0] * p3t1_28_p / np.pi,'darkgrey',alpha=0.7, linewidth=2 ,label=r'$z = 2.8 $, ptwise')
plt.plot( npd28.k , npd28.k/np.pi*(npd28.Pk) , 'k^', alpha=0.4, linewidth=2, label=r'$z=2.8$, npd')

plt.plot(p3t1_30[0]/dkM30(z30) , p3t1_30[0] * p3t1_30[1] / np.pi,'c',alpha=0.7, linewidth=2 ,label=r'$z = 3.0 $, th')
plt.plot(p3t1_30[0]/dkM30(z30) , p3t1_30[0] * p3t1_30_p / np.pi,'teal',alpha=0.7, linewidth=2 ,label=r'$z = 3.0 $, ptwise')
plt.plot( npd30.k , npd30.k/np.pi*(npd30.Pk) , 'c^', alpha=0.4, linewidth=2, label=r'$z=3.0$, npd')
